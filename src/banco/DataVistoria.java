...
public class DataVistoria {
    ...
    public void incluirVistoria(Vistoria vistoria) {
        try {
            ...
            statement.setInt(1, vistoria.getIdAgenda());
            statement.setInt(2, vistoria.getIdCorretor());
            statement.setDate(3, java.sql.Date.valueOf(vistoria.getDataVistoria()));
            statement.execute();
            statement.close();
    ...
    public Vistoria consultarVistoria(Vistoria vistoria) {
        try {
            String sql = "CALL consultar_Vistoria(?);";
            PreparedStatement statement = connection.getConnection().prepareStatement(sql);
