...
CREATE DEFINER=`root`@`localhost` PROCEDURE `inserir_Visita`(IN `v_idAgenda` INT, IN `v_idCorretor` INT, IN `v_dataVisita` DATE)
BEGIN
    INSERT INTO Visita (idAgenda, idCorretor, dataVisita, situacao) VALUES (v_idAgenda, v_idCorretor, v_dataVisita, 'Pendente');
END $$

CREATE DEFINER=`root`@`localhost` PROCEDURE `inserir_Vistoria`(IN `v_idAgenda` INT, IN `v_idCorretor` INT, IN `v_dataVistoria` DATE)
BEGIN
    INSERT INTO Vistoria (idAgenda, idCorretor, dataVistoria, resultado) VALUES (v_idAgenda, v_idCorretor, v_dataVistoria, 'Pendente');
END $$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `listar_Corretores` ()
